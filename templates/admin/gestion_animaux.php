            ...
            <div class="col-lg-3 mb-2" id="filtered_animal">
              <div class="border border-primary ">
                <form action="" method="POST" class="container" enctype="multipart/form-data">
                  ...
                  <div class="input-group mb-2">
                    <!-- IMAGE -->
                    <img src="<?= htmlspecialchars(BASE_URL . $animal['animal_visuel']) ?>" class="img-fluid img-fiche-animal" style="max-height: 88px;" onmouseover="this.style.maxHeight='100%';" onmouseout="this.style.maxHeight='88px';"><br />
                  ...
                  <div class="input-group mb-2">
                    ...
                    <input class="form-control form-control-sm" type="number" name="animal_age" id="animal_age" min="0" value="<?= htmlspecialchars($animal['animal_age']) ?>">
                  </div>
                  <!-- POIDS -->
                  <div class="input-group mb-2">
                    <label for="animal_poids" class="input-group-text input-group-text-sm">POIDS</label>
                    <input class="form-control form-control-sm" type="number" name="animal_poids" id="animal_poids" min="0" step="0.5" value="<?= htmlspecialchars($animal['animal_poids']) ?>">
                  </div>
                  <!-- TYPE NOURRITURE -->
                  <div class="input-group mb-2">
                    <label for="animal_food_id" class="input-group-text input-group-text-sm">NOURRITURE</label>
                    <input class="form-control form-control-sm" type="text" name="animal_food_id" id="animal_food_id" value="<?= htmlspecialchars($animal['food_type']) ?>" readonly>
                  </div>
                  <!-- UNITÉ NOURRITURE -->
                  <div class="input-group mb-2">
                    <label for="animal_unite_id" class="input-group-text input-group-text-sm">UNITE</label>
                    <input class="form-control form-control-sm" type="text" name="animal_unite_id" id="animal_unite_id" value="<?= htmlspecialchars($animal['unite_type']) ?>" readonly>
                  </div>
                  <!-- QUANTITÉ NOURRITURE -->
                  <div class="input-group mb-2">
                    <label for="animal_quantite_id" class="input-group-text input-group-text-sm">QUANTITE</label>
                    <input class="form-control form-control-sm" type="number" name="animal_quantite_id" id="animal_quantite_id" min="0" max="1000" value="<?= htmlspecialchars($animal['rapport_food_quantite']) ?>" readonly>
                  </div>
                  <!-- SANTÉ -->
                  <div class="input-group mb-2">
                    <label for="animal_etat_id" class="input-group-text input-group-text-sm">SANTE</label>
                    <input class="form-control form-control-sm" type="text" name="animal_etat_id" id="animal_etat_id" value="<?= htmlspecialchars($animal['etat_type']) ?>" readonly>
                  </div>
                  <!-- DOMAINE -->
                  <div class="input-group mb-2">
                    ...
                    <select class="form-select form-select-sm" id="animal_domaine_id" name="animal_domaine_id">
                      <?php foreach ($domaines as $domaine) : ?>
                        <option value="<?= htmlspecialchars($domaine['domaine_id']) ?>" <?php if ($animal['animal_domaine_id'] === $domaine['domaine_id']) : ?> selected <?php endif; ?>> <?= htmlspecialchars($domaine['domaine_name']) ?> </option>
                      <?php endforeach ?>
                    </select>
                  </div>
                  <!-- RACE -->
                  <div class="input-group mb-2">
                    ...
                    <select class="form-select form-select-sm" id="animal_race_id" name="animal_race_id">
                      <?php foreach ($races as $race) : ?>
                        <option value="<?= htmlspecialchars($race['race_id']) ?>" <?php if ($animal['animal_race_id'] === $race['race_id']) : ?> selected <?php endif; ?>><?= htmlspecialchars(ucfirst($race['race_nom'])) ?></option>
                      <?php endforeach ?>
                    </select>
                  </div>
